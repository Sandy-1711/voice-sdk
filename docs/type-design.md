...
_what each provider offers_; this one answers _what the shared types should be_
...
provider can stream audio _out_ of a one-shot request. Only ElevenLabs,
Deepgram and Cartesia accept text _in_ incrementally. OpenAI cannot — which is
...
**Context ids.** Cartesia _requires_ `context_id` on every message; ElevenLabs
...
| _heartbeat_        | `inactivity_timeout` query                          | **`{type:"KeepAlive"}` every ~10 s, mandatory** | same                 | ❌                             | ❌                                                                        |
...
2. **Delta vs cumulative is a real fork.** OpenAI and Cartesia send deltas,
   ...
   `STTTranscriptEvent` carries `text` _and_ `delta`, and `TurnTextTracker`
...
- **`Capabilities` as a static object.** It says _whether_ a provider does
  realtime TTS, not _which formats_ it accepts — so unsupported-format errors
